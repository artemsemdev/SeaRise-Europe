...
The production-static closure gate also starts from an empty browser profile,
warms the generated byte-sealed Flight shell, both verified search shards, and
one exact assessment, then removes the network and performs a document reload.
The reloaded app must render the full Flight interface, query the loaded search
index, and reproduce that assessment from verified storage. Selecting an
uncached scenario/horizon must produce the recoverable `connection-required`
technical state while preserving the previous accepted result. Independent
build inspection proves that the recursive Vite/lazy Worker/WASM closure is in
the shell and that neither PMTiles nor COG payloads were eagerly admitted.
