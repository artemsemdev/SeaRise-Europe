...
|---|---|---|
| `src/frontend/src/__tests__/stores/appStore.test.ts` | `src/web/src/domain/projection-state.test.ts`, `src/web/src/application/assessment-controller.test.ts`, and `src/web/src/application/use-assessment-runtime.test.tsx` cover every target phase, atomic accepted tuples, four outcomes, technical failures, stale work, cancellation, release replacement, retry, and reset. | Target unit boundary covered; App integration is still a blocking gate. |
| `src/frontend/src/__tests__/stores/mapStore.test.ts` | `src/web/src/components/map/MapExplorer.test.tsx` covers immutable coordinate selection and atomic overlay/band/text-alternative changes; `src/web/src/components/map/render-token.test.ts` guards stale map work. | Partial. Accepted projection, marker, layer, and quantile context must be proven synchronized in the production App journey before retirement. |
| `src/frontend/src/__tests__/stores/uiStore.test.ts` | `src/web/src/components/MethodologyDialog.test.tsx` covers controlled open/close, Escape, deterministic focus, focus return, release identity, and fail-closed content. | Target component boundary covered; App trigger wiring is still required. |
...
| `src/frontend/src/__tests__/components/LoadingState.test.tsx` | The phase table in `src/web/src/components/ProjectionPanel.test.tsx` covers booting, searching, evaluating, updating, and explicit status text. | App-level live-status journey is still required. |
| `src/frontend/src/__tests__/components/ErrorBanner.test.tsx` | `src/web/src/components/ProjectionPanel.test.tsx`, `src/web/src/components/SettlementSearch.test.tsx`, and `src/web/src/App.test.tsx` cover technical error presentation, retry routing, retry bounds, and the rule that technical failures are not scientific outcomes. | Covered at component boundaries; production failure journeys remain required. |
...
| `src/frontend/src/__tests__/components/ScenarioControl.test.tsx` and `src/frontend/src/__tests__/components/HorizonControl.test.tsx` | `src/web/src/components/ProjectionPanel.test.tsx` covers exactly three scenarios, exactly three horizons, active radio state, and one immutable selection command. | Add/retain keyboard E2E and all nine scenario/horizon journeys before retirement. |
| `src/frontend/src/__tests__/components/Legend.test.tsx` | `src/web/src/components/map/MapExplorer.test.tsx` covers atomic overlay/band/text-alternative changes and attribution. `src/web/src/components/map/MapExplorer.tsx` exposes all three quantile labels in text. | The binary exposure colour stops are intentionally not ported. Prove the accepted assessment tuple controls the production map/quantile context; PMTiles remains visual context only. |
| `src/frontend/src/__tests__/components/ResultPanel.test.tsx` | `src/web/src/components/ProjectionPanel.test.tsx` covers all and only the four ADR-024 outcomes, exact quantile/source metadata, limitations, attribution/licence, technical-state separation, dispositions, and prohibited wording. | Binary exposure assertions are intentionally not ported. Production journeys for four outcomes and technical failures are still required. |
