...
| `src/frontend/src/search/performance/browser-worker-evidence.test.ts` | `src/web/tests/static-shell.spec.ts`, `src/web/scripts/measure-local-candidate-search.mjs`, and `src/web/scripts/search-performance-evidence.test.mjs` | Replacement evidence is target-owned: committed-fixture Chromium and six deterministic evidence/filesystem cases cover exact manifest/receipt/shard/query binding, browser request privacy, explicit latency outcomes, nonclaims, canonical report identity, short writes, no overwrite, and identity-safe rollback. Retire the legacy suite only in the issue #70 deletion PR while these gates remain green. Candidate bytes and measurements remain local-only. |
