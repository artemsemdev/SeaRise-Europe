...
shard bytes in `browser-shards/`. The source projection remains the existing
synthetic settlement fixture at
`src/search/shards/fixtures/projection.synthetic.ndjson`. Its shard receipt
and both Brotli streams are validated byte-for-byte by the focused frontend
test before the report is accepted.
...
node --import tsx scripts/measure-settlement-browser-worker.ts validate \
  --projection /absolute/path/to/src/frontend/src/search/shards/fixtures/projection.synthetic.ndjson \
