...
1. write and validate the 51 data, configuration, receipt, rights, quality,
...
4. write `manifest.json` last, covering all 54 artifacts without hashing itself;
...
53 ordered checksum subjects and rejects any tree or file identity that changes
...
per artifact and 256 GiB for all 54 artifacts. Success is linearized when the
candidate pathname is reopened immediately before the last identity pass. The
search inventory contains two code-point-trie shards followed by their
receipt-last completion marker; omitting that marker or adding it outside the
inventory fails closed. The candidate descriptor remains open for the complete
pass. Matching identities observed
...
claim. The byte gate requires terminal write sequence 55 in the contract and a
